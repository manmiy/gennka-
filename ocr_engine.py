...
# 使用するGeminiモデル
# ------------------------------------------------------------------
# 2026/06/01 に gemini-2.0-flash / gemini-2.0-flash-lite は完全廃止されました。
# （廃止されたモデルを指定すると 404 Publisher Model not found エラーになります）
# 現行世代のモデルをデフォルトにしています。
# 注意: gemini-3.x系モデルは基本的に「global」リージョンでのみ利用可能です。
# 参考: https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/google-models
DEFAULT_MODEL_NAME = "gemini-3.5-flash-lite"

# サイドバー等で選択させる場合の候補一覧
AVAILABLE_MODELS = [
    "gemini-3.5-flash-lite",  # 最新世代・低コスト（globalリージョン推奨）
    "gemini-3.5-flash",       # 最新世代・バランス型（globalリージョン推奨）
    "gemini-3.1-flash-lite",  # 前世代・低コスト（globalリージョン推奨）
    "gemini-2.5-flash",       # 安定版・バランス型（2026/10/16 廃止予定）
    "gemini-2.5-flash-lite",  # 安定版・低コスト（2026/10/16 廃止予定）
    "gemini-2.5-pro",         # 安定版・高精度（2026/10/16 廃止予定）
]
...
def extract_invoice_data(
    ...
    Args:
        ...
        model_name: 使用するGeminiモデル名（省略時はDEFAULT_MODEL_NAME）
    ...
    try:
        ...
        # モデルが見つからない/使用不可などのエラーをそのまま呼び出し元に伝える。
        # よくある原因: モデル廃止（404）、リージョン未対応、権限不足など。
        raise RuntimeError(
            f"Gemini呼び出しでエラーが発生しました（model={model_name}）: {e}"
        ) from e
